...
def test_get_candidates(db: Session, create_user):
    """
    Create 15 users 
    users 1-5: have leaflet never received a leaflet.

    users 5-10: have a leaflet within a week old.

    users 10-15 have a leaflet more than a week old.
    """
    ...
    for n in range(1, 16):
        ...
        if 10 < n <= 15:
            ...
            db.add(leaflet)
            leaflet.user = user
            leaflet.created_at = datetime.now() - timedelta(days=random.randint(8, 14))
