...
@router.get("/")
def dashboard_get(request: Request, db: Session = Depends(get_db), current_user_id: str | None = Depends(current_user_id)):
...
def dashboard_leaflet_get(leaflet_id: int, request: Request, db: Session = Depends(get_db), current_user_id: str | None = Depends(current_user_id)):
    if not current_user_id:
        # Not logged in.
        return RedirectResponse("/signin")
...
def dashboard_recipe_get(recipe_id: int, request: Request, db: Session = Depends(get_db), current_user_id: str | None = Depends(current_user_id)):
    if not current_user_id:
        # Not logged in.
        return RedirectResponse("/signin")
